...
type MonitoringJob struct {
	...
	// The policy for determining the monitor's global status based on the status of the job in all regions
	Policy string `json:"policy"`

	// Controls behavior of how the job is assigned to monitoring regions. Currently this must be fixed — indicating monitoring regions are explicitly chosen.
	RegionScope string `json:"region_scope"`

	// Freeform notes to be included in any notifications about this job, e.g., instructions for operators who will receive the notifications.
	Notes string `json:"notes,omitempty"`

	//A free-form display name for the monitoring job.
	Name string `json:"name"`

	// Time(in seconds) between repeat notifications of a failed job. Set to 0 to disable repeating notifications.
